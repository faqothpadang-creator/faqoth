...
export default function Home() {
  // MESIN RADAR: Mendeteksi dan membaca semua file di dalam folder "posts"
  const postsDirectory = path.join(process.cwd(), 'posts');
  
  // Keamanan: Cek apakah folder posts ada untuk menghindari error
  let posts = [];
  if (fs.existsSync(postsDirectory)) {
    const fileNames = fs.readdirSync(postsDirectory);
    
    posts = fileNames.map((fileName) => {
      // Menghapus ekstensi ".md" untuk dijadikan tautan (slug)
      const slug = fileName.replace(/\.md$/, '');
      const fullPath = path.join(postsDirectory, fileName);
      const fileContents = fs.readFileSync(fullPath, 'utf8');
      
      // Mengambil data rahasia dari Frontmatter (Judul, Tanggal, Deskripsi)
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-20">
      {/* SECTION 1: HERO BANNER (Bagian Atas) */}
          ...
          <div className="relative z-10 px-6 py-16 md:px-12 w-full grid lg:grid-cols-2 gap-12 items-center">
            <div className="text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                {/* Tautan ini sekarang mengarah ke ID #tulisan di bawah */}
              ...
              <div className="relative bg-white rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="px-6 pb-8 relative">
                  <div className="w-28 h-28 mx-auto -mt-14 rounded-full border-4 border-white shadow-lg overflow-hidden bg-gray-200">
                    <img 
                      src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhnSx5Nf6Kjg0HZx033juFljVz3WLG11vnOtXnG9zGe49THSrC8s7YTMsJ5kMU_kmLOBYqMFBKNAmrN7aCFm54X7zfRv-6uovna0K5a2QSJts4iaklqfw5bDFuaRm1V8kZdTfQ3vci8q04tIkHZhW4Jbr1VsUmqelDLFUguzJA2JHHgG8xdmw0XMmHZl_w/s320/a.webp" 
                      alt="Raihan Faqoth" 
                      className="w-full h-full object-cover"
                    />
                  </div>
      ...
      {/* SECTION 2: DAFTAR ARTIKEL OTOMATIS (Bagian Bawah) */}
      <section id="tulisan" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        <div className="mb-10 text-center lg:text-left">
          <h2 className="text-3xl font-bold text-gray-900 inline-block border-b-4 border-[#1B4D3E] pb-2">
            Tulisan Terbaru
          </h2>
        </div>
          ...
          {posts.map((post) => (
            <Link href={`/artikel/${post.slug}`} key={post.slug} className="group">
              <article className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col h-full transform hover:-translate-y-1">
                {/* Visual Kotak Atas */}
                <div className="h-32 bg-[#1e293b] w-full flex items-center justify-center">
                  <i className="fas fa-book-open text-4xl text-white opacity-20"></i>
                </div>
                
                {/* Konten Kartu */}
                <div className="p-6 flex flex-col flex-grow">
                  <span className="text-sm font-semibold text-[#D4AF37] mb-2">{post.date}</span>
        ...
        {/* Pesan jika belum ada tulisan */}
        {posts.length === 0 && (
          <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
            <p className="text-gray-500">Belum ada tulisan yang dipublikasikan.</p>
          </div>
        )}
      </section>
